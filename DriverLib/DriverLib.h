...
namespace GuardianAngel
{
	...
	extern "C" {__declspec(dllexport) BOOL RegisterDriver(LPCTSTR DriverName, LPCTSTR PathName); }
	extern "C" {__declspec(dllexport) BOOL UnregisterDriver(LPCTSTR DriverName); }
	extern "C" {__declspec(dllexport) BOOL StartDriver(LPCTSTR DriverName); }
	extern "C" {__declspec(dllexport) BOOL StopDriver(LPCTSTR DriverName); }
	extern "C" {__declspec(dllexport) int GetInfo(unsigned char buffer[]); }
	extern "C" {__declspec(dllexport) int ReadPage(ULONG64 StartAddress, unsigned char buffer[]); }
